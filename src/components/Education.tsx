...
import { GraduationCap, BookOpen, ExternalLink, X, ZoomIn, ZoomOut, Move, Maximize2, Monitor, Smartphone } from 'lucide-react';
...
export default function Education() {
  ...
  const openThesisModal = () => {
    // Block mobile/tablet access completely
    if (isMobile) {
      setShowMobileMessage(true);
      return;
    }
    
    // Only open for desktop
  ...
  return (
    <>
                      ...
                      <h4 className="ml-2 text-lg font-semibold dark:text-white">
                        {index === 0 ? (
                          <button
                            ...
                            className="text-[#94c973] hover:text-[#7fb95e] transition-colors underline cursor-pointer flex items-center"
                          >
                            Thesis
                            {isMobile && <Smartphone className="h-4 w-4 ml-1 text-orange-500" />}
                          </button>
                        ) : (
                          'Thesis'
                        )}
      ...
      {/* Mobile/Tablet Restriction Message */}
      {showMobileMessage && (
            ...
            <div className="text-center">
              <div className="flex justify-center mb-4">
                <div className="relative">
                  <Monitor className="h-16 w-16 text-[#94c973]" />
              ...
              <p className="text-gray-600 dark:text-gray-300 mb-6">
                The thesis mind map is a high-resolution image that is only available on desktop computers for the best viewing experience.
              </p>
              
              <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 mb-6">
                <p className="text-sm text-blue-800 dark:text-blue-200">
                  <strong>To view the mind map:</strong>
                </p>
                <ul className="text-sm text-blue-700 dark:text-blue-300 mt-2 space-y-1 text-left">
                  <li>• Use a desktop or laptop computer</li>
                  <li>• Open this website in a desktop browser</li>
                  <li>• The mind map will open in full resolution</li>
                </ul>
              </div>
              
              <button
                onClick={closeMobileMessage}
                className="w-full px-4 py-2 bg-[#94c973] text-white rounded-lg hover:bg-[#7fb95e] transition-colors"
              >
                Got it
              </button>
            </div>
      ...
      {/* Desktop-Only Thesis Mind Map Modal */}
      {showThesisModal && !isMobile && (
        <div 
